...
const CategorySlider: React.FC<{ title: string, filteredMovies: any, description: string, currentUser?: SafeUser | null }> = ({ title, filteredMovies, description, currentUser }) => {
        ...
        <div className='flex flex-col h-full w-full gap-20'>
            <SkeletonWrapper isLoading={movies.isLoading}>
                {filteredMoviesByCategory.map(({ title, movies, description }) => (
                    <CategorySlider key={title} title={title} filteredMovies={movies} description={description} currentUser={currentUser} />
                ))}
            </SkeletonWrapper>
        </div>
    );
