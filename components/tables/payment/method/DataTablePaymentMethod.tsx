...
export function DataTablePaymentMethod({
  ...
  searchQuery,
  setSearchQuery
}: DataTablePaymentProps<PaymentCard, unknown>) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [filteredData, setFilteredData] = useState<PaymentCard[]>([]);

  useEffect(() => {
    const newData = data.filter((payment) =>
      payment.nameCard.toLocaleLowerCase().includes(searchQuery)
    );
